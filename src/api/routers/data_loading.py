...
from src.etl.loader import ETLLoader, ETLJobManager
...
class LoadRequest(BaseModel):
    ...
    filename: Optional[str] = Field(None, description="CSV filename (if not provided, uses default pattern)")
...
async def load_data(
    ...
    try:
        ...
        job_status = job_manager.create_job(job_id)
...
async def load_multiple_years(
    ...
    try:
        ...
        for fiscal_year in request.fiscal_years:
            # Create load request for each year
