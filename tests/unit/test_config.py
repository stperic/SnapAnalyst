...
class TestSettingsProperties:
    ...
    def test_is_development(self):
        ...
        with patch.dict('os.environ', {'ENVIRONMENT': 'development', 'DATABASE_URL': 'postgresql://localhost/test', 'SECRET_KEY': 'test-key'}):
            settings = Settings()
            assert settings.is_development is True

    def test_is_production(self):
        """Test is_production property"""
        with patch.dict('os.environ', {'ENVIRONMENT': 'production', 'DATABASE_URL': 'postgresql://localhost/test', 'SECRET_KEY': 'test-key'}):
            settings = Settings()
...
class TestDefaultLLMModel:
    """Test default_llm_model property"""

    def test_default_model_openai(self):
        """Test default model for OpenAI provider"""
        with patch.dict('os.environ', {'LLM_PROVIDER': 'openai', 'DATABASE_URL': 'postgresql://localhost/test', 'SECRET_KEY': 'test-key'}):
            settings = Settings()
    ...
    def test_default_model_anthropic(self):
        ...
        with patch.dict('os.environ', {'LLM_PROVIDER': 'anthropic', 'DATABASE_URL': 'postgresql://localhost/test', 'SECRET_KEY': 'test-key'}):
            settings = Settings()
            assert settings.default_llm_model == "claude-3-5-sonnet-20241022"

    def test_default_model_ollama(self):
        """Test default model for Ollama provider"""
        with patch.dict('os.environ', {'LLM_PROVIDER': 'ollama', 'DATABASE_URL': 'postgresql://localhost/test', 'SECRET_KEY': 'test-key'}):
            settings = Settings()
            assert settings.default_llm_model == "llama3.1:8b"

    def test_default_model_azure_openai(self):
        """Test default model for Azure OpenAI provider"""
        with patch.dict('os.environ', {'LLM_PROVIDER': 'azure_openai', 'DATABASE_URL': 'postgresql://localhost/test', 'SECRET_KEY': 'test-key'}):
            settings = Settings()
...
class TestSQLModel:
    ...
    def test_sql_model_openai_default(self):
        ...
        with patch.dict('os.environ', {'LLM_PROVIDER': 'openai', 'DATABASE_URL': 'postgresql://localhost/test', 'SECRET_KEY': 'test-key', 'LLM_SQL_MODEL': ''}, clear=False):
            settings = Settings()
            assert settings.sql_model == "gpt-4-turbo-preview"

    def test_sql_model_anthropic_default(self):
        """Test SQL model default for Anthropic"""
        with patch.dict('os.environ', {'LLM_PROVIDER': 'anthropic', 'DATABASE_URL': 'postgresql://localhost/test', 'SECRET_KEY': 'test-key', 'LLM_SQL_MODEL': ''}, clear=False):
            settings = Settings()
            assert settings.sql_model == "claude-3-5-sonnet-20241022"

    def test_sql_model_ollama_default(self):
        """Test SQL model default for Ollama"""
        with patch.dict('os.environ', {'LLM_PROVIDER': 'ollama', 'DATABASE_URL': 'postgresql://localhost/test', 'SECRET_KEY': 'test-key', 'LLM_SQL_MODEL': ''}, clear=False):
            settings = Settings()
            assert settings.sql_model == "llama3.1:8b"

    def test_sql_model_azure_openai_default(self):
        """Test SQL model default for Azure OpenAI"""
        with patch.dict('os.environ', {'LLM_PROVIDER': 'azure_openai', 'DATABASE_URL': 'postgresql://localhost/test', 'SECRET_KEY': 'test-key', 'LLM_SQL_MODEL': ''}, clear=False):
            settings = Settings()
            assert settings.sql_model == "gpt-4"
...
class TestKBModel:
    ...
    def test_kb_model_openai_default(self):
        ...
        with patch.dict('os.environ', {'LLM_PROVIDER': 'openai', 'DATABASE_URL': 'postgresql://localhost/test', 'SECRET_KEY': 'test-key', 'LLM_KB_MODEL': '', 'LLM_SQL_MODEL': ''}, clear=False):
            settings = Settings()
            assert settings.kb_model == "gpt-3.5-turbo"

    def test_kb_model_anthropic_default(self):
        """Test KB model default for Anthropic"""
        with patch.dict('os.environ', {'LLM_PROVIDER': 'anthropic', 'DATABASE_URL': 'postgresql://localhost/test', 'SECRET_KEY': 'test-key', 'LLM_KB_MODEL': '', 'LLM_SQL_MODEL': ''}, clear=False):
            settings = Settings()
            assert settings.kb_model == "claude-3-5-haiku-20241022"

    def test_kb_model_ollama_default(self):
        """Test KB model default for Ollama"""
        with patch.dict('os.environ', {'LLM_PROVIDER': 'ollama', 'DATABASE_URL': 'postgresql://localhost/test', 'SECRET_KEY': 'test-key', 'LLM_KB_MODEL': '', 'LLM_SQL_MODEL': ''}, clear=False):
            settings = Settings()
