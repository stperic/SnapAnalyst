...
class TestDataTransformer:
    ...
    def test_extract_members_sparse(self, fiscal_year: int):
        """Test member extraction with sparse data (not all 17 members)"""
        df = pl.DataFrame({
            "HHLDNO": ["TEST001"],
            "FSAFIL1": [1],
            "AGE1": [45],
            "FSAFIL2": [None],  # Member 2 doesn't exist
            "AGE2": [None],
            "FSAFIL3": [1],  # Member 3 exists
            "AGE3": [12],
        })
        ...
        # Should only extract members 1 and 3
        assert len(members_df) == 2
        assert list(members_df["member_number"]) == [1, 3]
    ...
    def test_extract_households_no_hhldno(self, fiscal_year: int):
        """Test household extraction when HHLDNO column doesn't exist"""
        # DataFrame without HHLDNO column
        df = pl.DataFrame({
            "STATE": ["CA", "TX"],
            "FSBEN": [284.50, 312.00],
            "CERTHHSZ": [2, 3],
        ...
        # Should generate case_id from row index (1, 2, 3...)
        assert len(households_df) == 2
        assert households_df["case_id"][0] == "1"
        assert households_df["case_id"][1] == "2"
        assert households_df["fiscal_year"][0] == fiscal_year

    def test_extract_members_no_members(self, fiscal_year: int):
        """Test member extraction when no members exist at all"""
        # DataFrame with no FSAFIL columns at all
        df = pl.DataFrame({
            "HHLDNO": ["TEST001"],
            "STATE": ["CA"],
        })
